...
try {
  if(!isset($_GET['openid_mode']) || $_GET['openid_mode'] == 'cancel') {
    ...
    if($openid->validate()) {
      ...
      $signature = all_inclusive_generate_signature($google_id);
      ?>
<html>
<head>
<script>
function init() {
  window.opener.wp_all_inclusive({'action' : 'all_inclusive', 'all_inclusive_provider' : 'google', 
    'all_inclusive_openid_identity' : '<?php echo $google_id ?>',
