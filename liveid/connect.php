...
$liveid_appid	= get_option('all_inclusive_liveid_appid_key');
$liveid_secret	= get_option('all_inclusive_liveid_secret_key');
$liveid_secalgo	= 'wsignin1.0';
$liveid_return	= all_inclusive_PLUGIN_URL . '/liveid/callback.php';
$liveid_policy	= get_option('all_inclusive_liveid_policy_url');
...
if ($consenttoken) {
  ...
  if($xml) {
    ...
    $signature = all_inclusive_generate_signature($liveid);
...
<script>
function init() {
  window.opener.wp_all_inclusive({'action' : 'all_inclusive', 'all_inclusive_provider' : 'liveid', 
    'all_inclusive_signature' : '<?php echo $signature ?>',
    'all_inclusive_liveid_identity' : '<?php echo $liveid ?>',
